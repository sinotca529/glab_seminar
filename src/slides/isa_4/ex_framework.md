...
\begin{align*}
    \llangle C, l' \rrangle = \text{case } C \text{ of} & \quad(\text{let } l \text{ be } \text{label}(C))\\
        \textbf{skip}                           \quad&:\quad \{ \text{next}(l) = l' \}\\
        C; C                                    \quad&:\quad \{ \text{next}(l) = l' \}\\
        x := E                                  \quad&:\quad \{ \text{next}(l) = l' \}\\
        \textbf{input}(x)                       \quad&:\quad \{ \text{next}(l) = \text{label}(C_1) \} \cup \llangle C_1, \text{label}(C_2) \rrangle \cup \llangle C_2, l' \rrangle\\
        \textbf{if}(B)\{C\} \textbf{else} \{C\} \quad&:\quad \{ \text{nextTrue}(l) = \text{label}(C_1), \text{nextFalse}(l) = \text{label}(C_2) \} \cup\\
                                                     &\quad\quad  \llangle C_1, l' \rrangle \cup \llangle C_2, l' \rrangle\\
        \textbf{while}(B) \{C\}                 \quad&:\quad \{ \text{nextTrue}(l) = \text{label}(C), \text{nextFalse}(l) = l' \} \cup \llangle C, l \rrangle\\
        \textbf{goto} E                         \quad&:\quad \{\}\quad (\text{to be determined at run-time by evaluating } E)\\
\end{align*}
$$
...
- $\mathbb{X}$ is the finite set of its variables.
- each statement of the program is uniquely labeled.
- $\text{next, nextTrue, nextFalse}$ is computed by $\llangle C, l_{\text{end}} \rrangle$.
...
$$
\begin{align*}
    \textbf{skip}                             \quad&:\quad (l, m) \hookrightarrow (\text{next}(l), m)\\
    \textbf{input}(x)                         \quad&:\quad (l, m) \hookrightarrow (\text{next}(l), \textit{update}_x(m, z)) && \text{for an input integer} z\\
    x := E                                    \quad&:\quad (l, m) \hookrightarrow (\text{next}(l), \textit{update}_x(m, \textit{eval}_E(m)))\\
    C_1; C_2                                  \quad&:\quad (l, m) \hookrightarrow (\text{next}(l), m)\\
    \textbf{if}(B)\{C_1\}\textbf{else}\{C_2\} \quad&:\quad (l, m) \hookrightarrow (\text{nextTrue}(l), \textit{filter}_B(m))\\
                                              \quad&:\quad (l, m) \hookrightarrow (\text{nextFalse}(l), \textit{filter}_{\neg B}(m))\\
    \textbf{while}(B) \{C\}                   \quad&:\quad (l, m) \hookrightarrow (\text{nextTrue}(l), \textit{filter}_B(m))\\
                                              \quad&:\quad (l, m) \hookrightarrow (\text{nextFalse}(l), \textit{filter}_{\neg B}(m))\\
    \textbf{goto} E                           \quad&:\quad (l, m) \hookrightarrow (\textit{eval}_E(m), m)\\
\end{align*}
$$
...
$$
\begin{align*}
    \text{Case: The } l \text{-labeled statement of}\\
    \textbf{skip}                             \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{next}(l), M^\sharp)\\
    \textbf{input}(x)                         \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{next}(l), \textit{update}_x^\sharp(M^\sharp, z)) && \text{for an input integer} z\\
    x := E                                    \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{next}(l), \textit{update}_x^\sharp(M^\sharp, \textit{eval}_E^\sharp(M^\sharp)))\\
    C_1; C_2                                  \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{next}(l), M^\sharp)\\
    \textbf{if}(B)\{C_1\}\textbf{else}\{C_2\} \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{nextTrue}(l), \textit{filter}_B^\sharp(M^\sharp))\\
                                              \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{nextFalse}(l), \textit{filter}_{\neg B}^\sharp(M^\sharp))\\
    \textbf{while}(B) \{C\}                   \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{nextTrue}(l), \textit{filter}_B^\sharp(M^\sharp))\\
                                              \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\text{nextFalse}(l), \textit{filter}_{\neg B}^\sharp(M^\sharp))\\
    \textbf{goto} E                           \quad&:\quad (l, M^\sharp) \hookrightarrow^\sharp (\textit{eval}_E^\sharp(M^\sharp), M^\sharp)\\
\end{align*}
$$
